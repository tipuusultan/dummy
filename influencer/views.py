...
def influencer_register_step2(request):
    if request.method == 'POST':
        ...
        Facebook_Page.objects.create(user=request.user, url=fb_url, category=fb_category, location='Hong Kong')
        Instagram_Page.objects.create(user=request.user, url=insta_url, category=insta_category, location='Hong Kong')
        YouTube_Channels.objects.create(user=request.user, url=yt_url, category=yt_category, location='Hong Kong')
